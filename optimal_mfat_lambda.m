...
%            Tr(j)=mfat_anal_final(r(j),D,x,alpha);
            Tr(j)=mfat_anal_lambda_fast2(r(j),D,lambda,alpha);
        end
        p = polyfit(r,Tr,2);
        r_min=-p(2)/(2*p(1));
        r0=r_min;
...
%4*lambda^2/D
%Tmin_tab
save('data_min_mfat_anal_lambda_fair_huge');
%% Testuj czas
r0_2=1/4;
tic
%y=mfat_anal_lambda(r0_2,1,1,2)
toc
tic
y=mfat_anal_lambda_fast2(r0_2,1,1,2)
toc
%% TESTUJ (skalowanie)!!
% Wyszedl pozytywnie dla x=5 i x=0.1
load('data_min_mfat_anal_lambda_naive_test01');
Tmin_tab5=Tmin_tab;
rmin_tab5=rmin_tab;
